...
def add_patient(connection):
    ...
    for _ in range(3):
        ...
        cursor.execute("""INSERT INTO patients (full_name, date_of_birth, gender, address) VALUES (%s, %s, %s, %s)""", (full_name, date_of_birth, gender, address))
        connection.commit()
...
def add_doctor(connection):
    ...
    for _ in range(5):
        ...
        cursor.execute("""INSERT INTO doctors (full_name, specialization, phone_number, email, years_of_experience) VALUES (%s, %s, %s, %s, %s)""", (full_name, specialization, phone_number, email, years_of_experience))
        connection.commit()
        print("5 doctors have been added.")

def add_appointment(connection):
    cursor = connection.cursor()
    # Thêm 3 cuộc hẹn cho 3 bệnh nhân (patient_id từ 1 đến 3)
    for patient_id in range(1, 4):
        ...
        cursor.execute("""INSERT INTO appointments (patient_id, doctor_id, appointment_date, reason) VALUES (%s, %s, %s, %s)""", (patient_id, doctor_id, appointment_date, reason))
        connection.commit()
        print("3 appointments have been added.")

def generate_report(connection):
    cursor = connection.cursor()
    cursor.execute("""SELECT p.full_name, p.date_of_birth, p.gender, p.address, d.full_name, a.reason, a.appointment_date
        FROM appointments a
...
def get_appointments_today(connection):
    ...
    cursor.execute("""SELECT p.full_name, p.date_of_birth, p.gender, d.full_name, a.status
        FROM appointments a
        JOIN patients p ON a.patient_id = p.patient_id
        JOIN doctors d ON a.doctor_id = d.doctor_id
        WHERE DATE(a.appointment_date) = %s""", (today,))        
    appointments = cursor.fetchall()
